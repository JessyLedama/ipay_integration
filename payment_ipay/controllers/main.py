...
class iPayController(http.Controller):
    _return_url = '/payment/ipay/return'
    _webhook_url = '/payment/ipay/webhook'

    @http.route(
        _return_url, type='http', auth='public', methods=['POST'], csrf=False, save_session=False
    )
    def ipay_return_from_checkout(self, **raw_data):
        """ Process the notification data sent by iPay after redirection from checkout.
        ...
        """
        _logger.info("handling redirection from iPay with data:\n%s", pprint.pformat(raw_data))
        ...
        tx_sudo._handle_notification_data('ipay', data)
        return request.redirect('/payment/status')

    @http.route(_webhook_url, type='http', auth='public', methods=['POST'], csrf=False)
    def ipay_webhook(self, **raw_data):
        """ Process the notification data sent by iPay to the webhook.
        ...
        :rtype: str
        """
        _logger.info("notification received from iPay with data:\n%s", pprint.pformat(raw_data))
        data = self._normalize_data_keys(raw_data)
        try:
            # Check the integrity of the notification
            received_signature = data.get('brq_signature')
    ...
    def _normalize_data_keys(data):
        ...
        As iPay parameters names are case insensitive, we can convert everything to lower-case
    ...
    def _verify_notification_signature(notification_data, received_signature, tx_sudo):
        ...
        expected_signature = tx_sudo.provider_id._ipay_generate_digital_sign(

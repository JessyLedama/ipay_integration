...
from odoo.addons.payment_ipay.const import STATUS_CODES_MAPPING
...
class PaymentTransaction(models.Model):
    ...
    def _get_specific_rendering_values(self, processing_values):
        """ Override of payment to return iPay-specific rendering values.
        ...
        if self.provider_code != 'ipay':
            return res

        return_url = urls.url_join(self.provider_id.get_base_url(), iPayController._return_url)
        rendering_values = {
            'api_url': self.provider_id._ipay_get_api_url(),
            'Brq_websitekey': self.provider_id.ipay_website_key,
        ...
        rendering_values['Brq_signature'] = self.provider_id._ipay_generate_digital_sign(
            rendering_values, incoming=False
        )
        return rendering_values

    def _get_tx_from_notification_data(self, provider_code, notification_data):
        """ Override of payment to find the transaction based on iPay data.
        ...
        if provider_code != 'ipay' or len(tx) == 1:
            return tx

        reference = notification_data.get('brq_invoicenumber')
        tx = self.search([('reference', '=', reference), ('provider_code', '=', 'ipay')])
        if not tx:
            raise ValidationError(
                "iPay: " + _("No transaction found matching reference %s.", reference)
            )

        return tx

    def _process_notification_data(self, notification_data):
        """ Override of payment to process the transaction based on iPay data.
        ...
        if not transaction_keys:
            raise ValidationError("iPay: " + _("Received data with missing transaction keys"))
        ...
        if status_code in STATUS_CODES_MAPPING['pending']:
            ...
            self._set_error("iPay: " + _("Unknown status code: %s", status_code))

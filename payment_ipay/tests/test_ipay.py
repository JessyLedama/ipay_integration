...
from odoo.addons.payment_ipay.controllers.main import iPayController
from odoo.addons.payment_ipay.tests.common import iPayCommon


@tagged('post_install', '-at_install')
class iPayTest(iPayCommon, PaymentHttpCommon):
    ...
    def test_redirect_form_values(self):
        ...
        self.assertEqual(form_info['action'], "https://testcheckout.ipayafrica.com/html/")
        self.assertDictEqual(expected_values, form_info['inputs'],
            "iPay: invalid inputs specified in the redirect form.")

    @mute_logger('odoo.addons.payment_ipay.models.payment_transaction')
    def test_feedback_processing(self):
        notification_data = iPayController._normalize_data_keys(self.sync_notification_data)
        tx = self._create_transaction(flow='redirect')
        tx._handle_notification_data('ipay', notification_data)
        self.assertEqual(tx.state, 'done')
        self.assertEqual(tx.provider_reference, notification_data.get('brq_transactions'))
        tx._handle_notification_data('ipay', notification_data)
        self.assertEqual(tx.state, 'done', 'iPay: validation did not put tx into done state')
        self.assertEqual(tx.provider_reference, notification_data.get('brq_transactions'))

        self.reference = 'Test Transaction 2'
        tx = self._create_transaction(flow='redirect')
        notification_data = iPayController._normalize_data_keys(dict(
            self.sync_notification_data,
            brq_invoicenumber=self.reference,
            brq_statuscode='2',
            brq_signature='b8e54e26b2b5a5e697b8ed5085329ea712fd48b2',
        ))
        self.env['payment.transaction']._handle_notification_data('ipay', notification_data)
        self.assertEqual(tx.state, 'error')

    @mute_logger('odoo.addons.payment_ipay.controllers.main')
    def test_webhook_notification_confirms_transaction(self):
        """ Test the processing of a webhook notification. """
        tx = self._create_transaction('redirect')
        url = self._build_url(iPayController._webhook_url)
        with patch(
            'odoo.addons.payment_ipay.controllers.main.iPayController'
    ...
    def test_webhook_notification_triggers_signature_check(self):
        ...
        url = self._build_url(iPayController._return_url)
        with patch(
            'odoo.addons.payment_ipay.controllers.main.iPayController'
            '._verify_notification_signature'
    ...
    def test_accept_notification_with_valid_signature(self):
        ...
        self._assert_does_not_raise(
            Forbidden,
            iPayController._verify_notification_signature,
    ...
    def test_signature_is_computed_based_on_lower_case_data_keys(self):
        ...
        computed_signature = self.provider._ipay_generate_digital_sign({
            'brq_a': '1',
            'brq_b': '2',
            'brq_c_first': '3',
            'brq_csecond': '4',
